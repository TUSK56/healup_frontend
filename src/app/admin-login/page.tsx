...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getAuthErrorMessage(e, t("errors.invalidCredentials")));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div dir={dir} style={{ minHeight: "100vh", background: "#eef0f5", color: "#1a2e4a", display: "flex", flexDirection: "column", fontFamily: "var(--font-cairo), Cairo, sans-serif" }}>
      {/* NAVBAR - swapped */}
      <nav style={{ background: "#fff", padding: "12px 36px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", flexDirection: dir === "rtl" ? "row-reverse" : "row" }}>
        <button type="button" style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", border: "1.5px solid #c8cfd8", borderRadius: 20, background: "#fff", fontFamily: "var(--font-cairo), Cairo, sans-serif", fontSize: 13.5, fontWeight: 700, color: "#1a2e4a", cursor: "pointer", direction: dir }}>
          <svg viewBox="0 0 24 24" width={16} height={16} fill="#1a2e4a"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17h-2v-2h2v2zm2.07-7.75l-.9.92C13.45 12.9 13 13.5 13 15h-2v-.5c0-1.1.45-2.1 1.17-2.83l1.24-1.26c.37-.36.59-.86.59-1.41 0-1.1-.9-2-2-2s-2 .9-2 2H8c0-2.21 1.79-4 4-4s4 1.79 4 4c0 .88-.36 1.68-.93 2.25z"/></svg>
          <span>{t("auth.adminLogin.help")}</span>
        </button>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexDirection: "row" }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: "#3678BE" }}>{t("auth.adminLogin.dashboardBadge")}</span>
        </div>
      </nav>
      {/* MAIN - swapped boxes */}
      <main style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
        <div style={{ display: "flex", flexDirection: "row", width: "100%", maxWidth: 900, borderRadius: 20, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.15), 0 8px 24px rgba(0,0,0,0.10)" }}>
          {/* Form Panel - now left */}
          <form onSubmit={handleSubmit} style={{ flex: 1, background: "#fff", padding: "48px 44px 36px", display: "flex", flexDirection: "column", direction: dir }}>
            <div style={{ textAlign: dir === "rtl" ? "right" : "left", marginBottom: 28 }}>
              <h1 style={{ fontSize: 26, fontWeight: 900, color: "#1a2e4a", marginBottom: 8 }}>{t("auth.adminLogin.title")}</h1>
              <p style={{ fontSize: 13, color: "#9aa3b0", fontWeight: 400, lineHeight: 1.7 }}>{t("auth.adminLogin.subtitle")}</p>
            </div>
            <div style={{ marginBottom: 18 }}>
              <span style={{ display: "block", marginBottom: 8, fontSize: 12.5, fontWeight: 700, color: "#1a2e4a" }}>{t("auth.adminLogin.username")}</span>
              <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder={t("auth.adminLogin.usernamePlaceholder")} style={{ width: "100%", padding: "13px 42px 13px 16px", border: "1.5px solid #dde3ed", borderRadius: 10, fontFamily: "var(--font-cairo), Cairo, sans-serif", fontSize: 13.5, color: "#1a2e4a", background: "#fff", outline: "none", textAlign: dir === "rtl" ? "right" : "left", direction: dir, transition: "border-color 0.2s" }} />
            ...
            <div style={{ marginBottom: 18 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 12.5, fontWeight: 700, color: "#1a2e4a" }}>{t("auth.password")}</span>
                <a href="#" style={{ fontSize: 12, fontWeight: 700, color: "#2356c8", textDecoration: "none" }}>{t("auth.forgotPassword")}</a>
              </div>
              <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth.passwordPlaceholder")} id="adminPass" style={{ width: "100%", padding: "13px 42px 13px 16px", border: "1.5px solid #dde3ed", borderRadius: 10, fontFamily: "var(--font-cairo), Cairo, sans-serif", fontSize: 13.5, color: "#1a2e4a", background: "#fff", outline: "none", textAlign: dir === "rtl" ? "right" : "left", direction: dir, transition: "border-color 0.2s" }} />
                <span style={{ position: "absolute", right: 13, pointerEvents: "none", display: "flex", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "flex-start", gap: 8, marginBottom: 22, direction: dir, textAlign: dir === "rtl" ? "right" : "left" }}>
              ...
              <label htmlFor="remember" style={{ fontSize: 13, color: "#1a2e4a", fontWeight: 500, cursor: "pointer" }}>
                {t("auth.rememberMe")}
              </label>
            </div>
          ...
          <div style={{
            ...
            <div style={{ zIndex: 1, textAlign: dir === "rtl" ? "right" : "left" }}>
              ...
              <h2 style={{ fontSize: 26, fontWeight: 900, color: "#0456AE", marginBottom: 14, lineHeight: 1.4 }}>{t("auth.adminLogin.panelWelcome")}</h2>
              <p style={{ fontSize: 13, color: "#3678BE", lineHeight: 1.9, fontWeight: 400 }}>{t("auth.adminLogin.panelText", { brand: t("common.brand") })}</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-start", gap: 20, zIndex: 1, direction: dir }}>
                <div style={{ display: "flex", flexDirection: "row-reverse", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 700, color: "#1a2e4a" }}>
                {t("auth.adminLogin.encryptedConnection")}
              ...
              <div style={{ display: "flex", flexDirection: "row-reverse", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 700, color: "#1a2e4a" }}>
                {t("auth.adminLogin.dataProtection")}
      ...
      <footer style={{ textAlign: "center", padding: 16, fontSize: 12.5, color: "#9aa3b0", direction: dir }}>{t("auth.adminLogin.footer", { year })}</footer>
    </div>
  );
}
