              ...
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <input
                    ...
                    placeholder="••••••••"
                    style={{ width: '100%', padding: isAr ? "13px 46px 13px 16px" : "13px 16px 13px 46px", border: '1.5px solid #dde3ed', borderRadius: 10, fontFamily: 'Cairo, sans-serif', fontSize: 14, color: '#1a2e4a', background: '#fff', outline: 'none', textAlign: isAr ? "right" : "left", direction: isAr ? "rtl" : "ltr", transition: 'border-color 0.2s' }}
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', [isAr ? "left" : "right"]: 14, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                    {showPassword ? (
