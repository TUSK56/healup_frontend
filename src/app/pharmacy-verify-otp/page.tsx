      ...
      <body style={{ fontFamily: 'Cairo, sans-serif', background: '#eef0f5', color: '#1a2e4a', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: '#fff', borderRadius: 20, padding: '44px 48px 40px', width: '100%', maxWidth: 480, boxShadow: '0 6px 32px rgba(0,0,0,0.08)', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 14, marginBottom: 28, direction: 'ltr' }}>
              {otp.map((digit, idx) => (
                <input
                  ...
                  style={{ width: 64, height: 64, border: '1.5px solid #dde3ed', borderRadius: 12, fontFamily: 'Cairo, sans-serif', fontSize: 22, fontWeight: 700, color: '#1a2e4a', textAlign: 'center', outline: 'none', background: '#fff', transition: 'border-color 0.2s, box-shadow 0.2s', caretColor: '#2356c8', borderColor: digit ? '#2356c8' : '#dde3ed', borderWidth: digit ? 2 : 1.5 }}
                />
              ))}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 12.5, color: '#9aa3b0', direction: 'ltr' }}>
            ...
            يمكنك طلب كود جديد خلال {`00:${timer.toString().padStart(2, '0')}`}
          </div>
          <a href="/pharmacy-forgot-password" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 14, fontWeight: 700, color: '#2356c8', textDecoration: 'none', direction: 'rtl', marginTop: 18 }}>
