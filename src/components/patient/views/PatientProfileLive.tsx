...
import { User, Bell, MapPin, Shield, Trash2, Home, Briefcase, Plus, Lock, CheckCircle2, Camera, type LucideIcon } from "lucide-react";
...
const SectionHeader = ({ icon: Icon, title, action }: { icon: LucideIcon; title: string; action?: React.ReactNode }) => (
...
export default function PatientProfileLive() {
  return (
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-right">
              <div className="relative group">
                <div className="w-32 h-32 rounded-full border-4 border-slate-50 overflow-hidden shadow-lg">
                  <img src="https://picsum.photos/seed/profile/200/200" alt="Profile" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                </div>
                <button type="button" className="absolute bottom-1 right-1 p-2 bg-healup-blue text-white rounded-full shadow-md hover:scale-110 transition-transform">
                  <Camera size={16} />
                </button>
              </div>
          ...
          <div className="lg:col-span-12 w-full space-y-8" style={{ maxWidth: 1120, marginRight: 0, marginLeft: "auto" }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input type="text" defaultValue="01012345678" className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-healup-blue/20 focus:border-healup-blue transition-all text-slate-700 text-left" dir="ltr" />
                    <div className="w-20 px-3 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-slate-500 text-center font-bold">+20</div>
                  </div>
                </div>
            ...
            <Card className="p-8 min-h-[250px]">
              <SectionHeader
                ...
                action={
                  <button type="button" className="text-healup-blue font-bold flex items-center gap-1 hover:underline">
                    <Plus size={18} />
              ...
              <div className="space-y-4">
                {[{ id: 1, type: "المنزل", address: "المعادي، القاهرة، شارع 9", icon: Home }, { id: 2, type: "العمل", address: "مدينة نصر، القاهرة، الحي السابع", icon: Briefcase }].map((addr) => (
                  <div key={addr.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-healup-blue/30 transition-colors group">
                    ...
                    <button type="button" className="p-2 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100">
                      <Trash2 size={18} />
                    </button>
                  </div>
                ))}
            ...
            <Card className="p-8">
              ...
              <div className="p-6 border-2 border-dashed border-slate-100 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-6">
                ...
                <button type="button" className="px-6 py-2.5 bg-healup-light-blue text-healup-blue font-bold rounded-xl hover:bg-healup-blue hover:text-white transition-all">
                  تغيير كلمة المرور
                </button>
              </div>
            </Card>
            ...
            <div className="flex items-center justify-end gap-4 pt-4">
              <button type="button" className="flex-1 sm:flex-none px-10 py-4 bg-white text-slate-500 font-bold rounded-2xl border border-slate-200 hover:bg-slate-50 transition-all">
                إلغاء
              </button>
              <button type="button" className="flex-1 sm:flex-none px-10 py-4 bg-healup-blue text-white font-bold rounded-2xl shadow-lg shadow-healup-blue/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
