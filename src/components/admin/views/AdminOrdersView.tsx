...
function toArabicDateTime(value?: string) {
  if (!value) return "-";
...
function statusLabel(status: string) {
  switch ((status || "").toLowerCase()) {
    case "pending_pharmacy_confirmation":
      return { text: "بانتظار تأكيد الصيدلية", cls: "bg-[#FEF3C7] text-[#AB6D3E]" };
    case "confirmed":
      return { text: "تم التأكيد", cls: "bg-[#DBEAFE] text-[#2244B8]" };
    case "preparing":
      return { text: "جاري التحضير", cls: "bg-[#E0E7FF] text-[#3930A7]" };
    case "out_for_delivery":
      return { text: "قيد التوصيل", cls: "bg-[#E0E7FF] text-[#3930A7]" };
    case "ready_for_pickup":
      return { text: "جاهز للاستلام", cls: "bg-[#DBEAFE] text-[#2244B8]" };
    case "completed":
      return { text: "تم التسليم", cls: "bg-[#D1FAE5] text-[#107057]" };
    case "rejected":
      return { text: "مرفوض", cls: "bg-[#FFE4E6] text-[#A81F48]" };
    default:
      return { text: status || "غير معروف", cls: "bg-slate-100 text-slate-600" };
  }
}

function locationText(order: AdminOrder) {
    ...
    <div className="flex min-w-0 flex-1 flex-col bg-[#F8FAFC]">
      <main className="w-full min-w-0 px-6 py-8 sm:px-8 sm:py-10 lg:px-10">
        ...
        <div className="mb-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "إجمالي الطلبات", value: String(stats.total), change: null, icon: ShoppingBag, bgColor: "#E5EEF7", textColor: "#0456AE" },
            { label: "طلبات نشطة", value: String(stats.active), change: null, icon: Clock, bgColor: "#FEF5E6", textColor: "#AB6D3E" },
            { label: "طلبات مكتملة", value: String(stats.completed), change: null, icon: CheckCircle2, bgColor: "#E7F8F2", textColor: "#107057" },
            { label: "طلبات ملغاة", value: String(stats.cancelled), change: null, icon: XCircle, bgColor: "#FEEBEF", textColor: "#A81F48" },
          ].map((stat, idx) => (
            <motion.div
              key={stat.label}
            ...
            >
              ...
              <p className="mt-1 text-3xl font-bold text-slate-900">{Number(stat.value).toLocaleString("ar-EG")}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                  ...
                  pagedRows.map((order, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-5 text-sm text-slate-700">{toArabicDateTime(order.created_at)}</td>
                        <td className="px-6 py-5 text-sm text-slate-600">{locationText(order)}</td>
                        <td className="px-6 py-5 font-bold text-slate-900">{Number(order.total_price || 0).toLocaleString("ar-EG")} ج.م</td>
          ...
          <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-100 p-6 sm:flex-row">
            <p className="text-sm text-slate-500">
              ...
              <span className="font-bold text-slate-900">{filtered.length.toLocaleString("ar-EG")}</span> طلب
            </p>
